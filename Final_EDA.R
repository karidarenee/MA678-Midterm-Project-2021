...
p1<-ggplot(HR, aes(x = log(training_hours))) +
...
p6<-HR_edu%>% 
  ...
  mutate(target = factor(target)) %>%
  drop_na() %>%
  ggplot(aes(company_type, ratio, fill = target)) + 
  geom_bar(stat = "identity", color = 'black') +
  scale_fill_manual(values = c('darkolivegreen3', 'lightgoldenrod1'),
                    labels = c("Not searching for a new job", 
                               "Searching for a new job")) +
...
p_2v<-
  ...
  ggplot() + 
  geom_point(aes(x = company_size,y = mean, color = company_size),size=2) + 
  theme_bw() + 
  xlab("company size") +   ylab("target") + 
  ggtitle("Target under different education level and company size") + 
  facet_wrap(~ education_level) +
  theme(axis.text.x = element_text(angle = 50, hjust = 0.5, vjust = 0.5))+
  theme(plot.title=element_text(size=12, face="bold", hjust = 0.5))

p_2v2<-
  HR%>% group_by(company_type, education_level) %>%
  summarize(mean = mean(target)) %>% ungroup() %>%
  ggplot() + 
  geom_point(aes(x = company_type,y = mean, color = company_type),size=2) + 
  theme_bw() + 
  xlab("company type") +   ylab("target") + 
  ggtitle("Target under different education level and company type") + 
  facet_wrap(~ education_level) +
  theme(axis.text.x = element_text(angle = 50, vjust = 0.5))+
  theme(plot.title=element_text(size=12, face="bold", hjust = 0.5))

p_2v3<-
  HR%>% group_by(company_type, enrolled_university) %>%
  summarize(mean = mean(target)) %>% ungroup() %>%
  ggplot() + 
  geom_point(aes(x = company_type,y = mean, color = company_type),size=2) + 
  theme_bw() + 
  xlab("company type") +   ylab("target") + 
  ggtitle("Target under different enrolled university and company type") + 
  facet_wrap(~ enrolled_university) +
  theme(axis.text.x = element_text(angle = 50, vjust = 0.5))+
  theme(plot.title=element_text(size=12, face="bold", hjust = 0.5))

line1<-ggplot(data = HR)+
  aes(y = target, x = log(training_hours))+
...
line3<-ggplot(data = HR)+
  aes(y = target, x = log(training_hours))+
  geom_point(aes(color = company_type),alpha = 0.3)+
  labs(title="Relationship between city development index & job change\n by company type and enrolled university",
       x="City Development Index", y = "Target")+
  geom_smooth(aes(color = company_type),method = "lm",se=F)+
  theme(axis.text.x = element_text(angle = 50, hjust = 0.5, vjust = 0.5))+
  facet_grid(~enrolled_university)

line10<-ggplot(data = HR)+
  aes(y = target, x = city_development_index)+
  geom_point(aes(color = company_type),alpha = 0.3)+
...
line4<-ggplot(data = HR)+
  aes(y = target, x = experience)+
  geom_point(aes(color = company_size),alpha = 0.3)+
  labs(title="Relationship between working experience & intention to leave\n by company size and education level",
       x="Experience", y = "Target")+
  geom_smooth(aes(color = company_size),method = "lm",se=F)+
  theme(axis.text.x = element_text(angle = 50, hjust = 0.5, vjust = 0.5))

line5<-ggplot(data = HR)+
  aes(y = target, x = city_development_index)+
  geom_point(aes(color = company_type),alpha = 0.3)+
  labs(title="Relationship between city development index & job change\n by company type",
       x="city development index", y = "Target")+
...
line6<-ggplot(data = HR)+
  aes(y = target, x = city_development_index)+
  geom_point(aes(color = education_level),alpha = 0.3)+
  labs(title="Relationship between city development index & job change\n by education level",
       x="city development index", y = "Target")+
  geom_smooth(aes(color = education_level),method = "lm",se=F)+
  theme(axis.text.x = element_text(angle = 50, hjust = 0.5, vjust = 0.5))

line7<-ggplot(data = HR)+
  aes(y = target, x = city_development_index)+
  geom_point(aes(color = enrolled_university),alpha = 0.3)+
  labs(title="Relationship between city development index & job change\n by enrolled university",
       x="city development index", y = "Target")+
  geom_smooth(aes(color = enrolled_university),method = "lm",se=F)+
  theme(axis.text.x = element_text(angle = 50, hjust = 0.5, vjust = 0.5))
